...
- Consider edge $\color{brown}(y,x)$ with $y \in {\color{skyblue}T}, x \in {\color{magenta}S}$
    - ${\color{red}f(y,x)}={\color{forestgreen}c(u,v)}$ because otherwise the back edge ${\color{brown}w(y,x)}>0$ in $G_{\color{red}f}$, which would mean $y \in {\color{magenta}S}$
...
```{dot}
// this duplicates the one in the maxflow column
digraph graph25 {
